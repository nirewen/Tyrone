...
export const CanvasUtils = {
    ...
    getPolygonImage: async function(url, x, sides, startAngle, style) {
        if (url) {
            ...
            img.src = await request({url, encoding: null});

            let canvas = new Canvas(x, x),
                ctx = canvas.getContext('2d');
    ...
    sharpen: function(canvas, mix = 0.9) {
        ...
        let ctx = canvas.getContext('2d'),
        	w = canvas.width,
        	h = canvas.height,
        	x, sx, sy, r, g, b, dstOff, srcOff, a, wt, cx, cy, scy, scx,
        ...
        while (y--) {
            ...
            while (x--) {
                ...
                // definindo o alpha pro q já tava, mas pode ser afiado tb
