...
export class Player extends Lavalink.Player {
    constructor (options = {}) {
        super(options)
    ...
    next (user) {
        if (this._loop) 
            this.queueTrack(this.playingSong, true)
    ...
    bassboost (state = true) {
        ...
        if (state) {
            ...
            this.volume(150)
            this.setEQ(Array(6).fill(0).map((_n, i) => ({ band: i, gain: 1 })))
            return true
        }
        ...
        this.setEQ(Array(6).fill(0).map((_n, i) => ({ band: i, gain: 0 })))
